...
def torch_cross_entropy(batch_size, seq_length, vocab_size, logits_scale, seed):
    ...
    identity = IdentityLayer((batch_size, seq_length, vocab_size), scale=logits_scale).to(
        get_accelerator().device_name()
    )
    logits = identity()
    target = get_accelerator().LongTensor(size=(batch_size, seq_length)).random_(0, vocab_size)
    loss = (
