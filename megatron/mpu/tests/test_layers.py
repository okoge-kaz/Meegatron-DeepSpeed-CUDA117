...
def test_parallel_embedding(tensor_model_parallel_size):
    ...
    set_random_seed(123)
    input_data = (
        torch.LongTensor(size=(batch_size, seq_length)).random_(0, vocab_size).to(device_name)
    )
...
def parallel_self_attention(
    ...
    # Network
    identity_layer = IdentityLayer3D(batch_size, sequence_length, hidden_size).to(device_name)
    attention_layer = mpu.BertParallelSelfAttention(hidden_size, num_att_heads, dropout_prob).to(
        device_name
    )
