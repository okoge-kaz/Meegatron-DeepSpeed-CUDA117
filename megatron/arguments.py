...
def _add_training_args(parser):
    ...
    group.add_argument("--random-ltd", action="store_true", help="enable random layer token drop")
    group.add_argument(
    ...
    group.add_argument(
        "--ds-inference", action="store_true", help="DeepSpeed inference engine being used"
    )
    group.add_argument("--cpu-optimizer", action="store_true", help="Run optimizer on CPU")
    group.add_argument(
        "--cpu_torch_adam", action="store_true", help="Use Torch Adam as optimizer on CPU."
    )
    group.add_argument(
        "--no-pipeline-parallel", action="store_true", help="Disable pipeline parallelism"
    )
    group.add_argument("--use-tutel", action="store_true", help="Use Tutel optimization for MoE")
    group.add_argument(
        "--inference",
        action="store_true",
...
def _add_data_args(parser):
    ...
    group.add_argument(
        "--aml-data-download-path", type=str, default=None, help="Path to mounted input dataset"
    )
    group.add_argument(
        "--data-path",
        nargs="*",
    ...
    group.add_argument(
        "--return-data-index", action="store_true", help="Return the index of data sample."
    )
    group.add_argument(
    ...
    group.add_argument(
        "--train-idx-path", type=str, default=None, help="Force to use certain index file."
    )
    group.add_argument(
        "--train-doc-idx-path", type=str, default=None, help="Force to use certain index file."
    )
    group.add_argument(
        "--train-sample-idx-path", type=str, default=None, help="Force to use certain index file."
    )
    group.add_argument(
        "--train-shuffle-idx-path", type=str, default=None, help="Force to use certain index file."
    )
    return parser
...
def _add_vit_args(parser):
    ...
    group.add_argument(
        "--img-dim", type=int, default=224, help="Image size for vision classification task"
    )
    group.add_argument(
...
def _add_zero_args(parser):
    ...
    group.add_argument(
        "--zero-reduce-scatter", action="store_true", help="Use reduce scatter if specified"
    )
    group.add_argument(
...
def _add_activation_checkpoint_args(parser):
    ...
    group.add_argument(
        "--checkpoint-in-cpu", action="store_true", help="Move the activation checkpoints to CPU."
    )
    group.add_argument(
...
def _add_distillation_args(parser):
    group = parser.add_argument_group("Knowledge distillation", "Distillation Configurations")
    ...
    group.add_argument(
        "--hidden-size-teacher", type=int, default=None, help="Tansformer teacher hidden size."
    )
    group.add_argument(
    ...
    group.add_argument(
        "--mos", action="store_true", help="Enable Mixture-of-Students via knolwedge distillation."
    )
    group.add_argument("--kd", action="store_true", help="Enable knolwedge distillation.")
    ...
    group.add_argument("--reset-iteration", action="store_true", help="Reset the iteration count.")
