...
def pretrain(
    train_valid_test_dataset_provider: Callable[[int], Tuple[torch.utils.data.Dataset, ...]],
    model_provider,
    forward_step_func,
    ...
    if args.deepspeed:
        ...
        if (
            ...
            and "enabled" in args.deepspeed_configuration["curriculum_learning"]
        ):
            args.curriculum_learning_legacy = args.deepspeed_configuration["curriculum_learning"][
                "enabled"
            ]
    ...
    if args.do_test:
        ...
        evaluate_and_print_results(
            prefix, forward_step_func, test_data_iterator, model, 0, True, test=True
        )
...
def load_model_weights_only(model_provider_func):
    ...
    if args.deepspeed:
        ...
        model, optimizer, _, lr_scheduler = deepspeed.initialize(model=model[0], config=ds_config)
...
def setup_model_and_optimizer(
    ...
    if args.kd or args.mos:
        model, _, _, _ = deepspeed.initialize(
            model=model[0], args=args, mpu=mpu if args.no_pipeline_parallel else None
        )
        model = [model]
        if args.load is not None:
...
def training_log(
    ...
    for key in loss_dict:
        if not skipped_iter:
            total_loss_dict[key] = (
                total_loss_dict.get(key, get_accelerator().FloatTensor([0.0])) + loss_dict[key]
            )
        else:
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0) and is_last_rank():
        ...
        if args.log_learning_rate_to_tensorboard:
            ...
            writer.add_scalar(
                "learning-rate/learning-rate vs tokens", learning_rate, args.consumed_train_tokens
            )
        if args.log_batch_size_to_tensorboard:
            writer.add_scalar("batch-size/batch-size", batch_size, iteration)
            writer.add_scalar(
                "batch-size/batch-size vs samples", batch_size, args.consumed_train_samples
            )
        for key in loss_dict:
        ...
        if args.log_loss_scale_to_tensorboard:
            ...
            writer.add_scalar(
                "loss-scale/loss-scale vs samples", loss_scale, args.consumed_train_samples
            )
            writer.add_scalar(
                "loss-scale/loss-scale vs tokens", loss_scale, args.consumed_train_tokens
            )
        ...
        if num_zeros_in_grad is not None:
            ...
            writer.add_scalar(
                "num-zeros/num-zeros vs samples", num_zeros_in_grad, args.consumed_train_samples
            )
            writer.add_scalar(
                "num-zeros/num-zeros vs tokens", num_zeros_in_grad, args.consumed_train_tokens
            )
        if params_norm is not None:
            writer.add_scalar("params-norm/params-norm", params_norm, iteration)
            writer.add_scalar(
                "params-norm/params-norm vs samples", params_norm, args.consumed_train_samples
            )
            writer.add_scalar(
                "params-norm/params-norm vs tokens", params_norm, args.consumed_train_tokens
            )
        ...
        if args.curriculum_learning_legacy or args.data_efficiency_curriculum_learning:
            writer.add_scalar("seqlen/curriculum_seqlen", args.curriculum_seqlen, iteration)
            writer.add_scalar(
        ...
        if args.random_ltd:
            writer.add_scalar(
                "seqlen/random_ltd_reserved_length", args.random_ltd_reserved_length, iteration
            )
            writer.add_scalar(
    ...
    if iteration % args.tensorboard_log_interval == 0:
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if writer and is_last_rank():
                ...
                writer.add_scalar(
                    "optimizer/variance_l1 vs tokens", opt_stats[4], args.consumed_train_tokens
                )
                writer.add_scalar(
                ...
                writer.add_scalar(
                    "optimizer/momentum_l1 vs tokens", opt_stats[6], args.consumed_train_tokens
                )
                writer.add_scalar(
                    "optimizer/weight_l1 vs tokens", opt_stats[7], args.consumed_train_tokens
                )
                writer.add_scalar(
    ...
    while iteration < args.train_iters and (
        ...
        if args.random_ltd:
            ...
            if args.random_ltd_reserved_length < args.actual_seq_length:
                args.actual_seq_length = (
                    args.actual_seq_length * (args.num_layers - args.random_ltd_layer_num)
        ...
        if args.curriculum_learning_legacy or args.data_efficiency_curriculum_learning:
            if hasattr(args, "data_efficiency_curriculum_learning_numel"):
                ...
                args.consumed_train_tokens += (
                    mpu.get_data_parallel_world_size() * get_num_microbatches() * act_token
                )
            else:
        ...
        if args.exit_duration_in_mins:
            ...
            done_cuda = get_accelerator().IntTensor([train_time > args.exit_duration_in_mins])
...
def evaluate(forward_step_func, data_iterator, model, verbose=False):
    ...
    if args.curriculum_learning_legacy and not args.no_pipeline_parallel:
        ...
        args.curriculum_seqlen = args.curriculum_scheduler.update_difficulty(args.iteration + 1)
        if args.curriculum_seqlen < args.seq_length:
            model[0].reset_activation_shape()
...
def evaluate_and_print_results(
    prefix, forward_step_func, data_iterator, model, iteration, verbose=False, test=False
):
    ...
    for key in total_loss_dict:
        ...
        if writer and is_last_rank():
            ...
            writer.add_scalar(
                f"lm-loss-validation/{key} {data_type}", total_loss_dict[key].item(), iteration
            )
            writer.add_scalar(
            ...
            if args.log_validation_ppl_to_tensorboard:
                writer.add_scalar(f"lm-loss-validation/{key} {data_type} ppl", ppl, iteration)
                writer.add_scalar(
...
def build_train_valid_test_data_iterators(build_train_valid_test_datasets_provider):
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        flags = get_accelerator().LongTensor([int(do_train), int(do_valid), int(do_test)])
    else:
