    ...
    if mpu.is_pipeline_last_stage():
        if args.finetune and args.task in ["CoLA", "STS-B"]:
            if args.task == "CoLA":
                ...
                predicted_gather = [
                    torch.zeros(len(predicted), dtype=torch.long, device=predicted.device)
                    for _ in range(mpu.get_data_parallel_world_size())
                ]
            else:
                ...
                labels_gather = [
                    torch.zeros(len(labels), dtype=torch.float, device=labels.device)
                    for _ in range(mpu.get_data_parallel_world_size())
                ]
                predicted_gather = [
                    torch.zeros(len(predicted), dtype=torch.float, device=predicted.device)
                    for _ in range(mpu.get_data_parallel_world_size())
                ]
            torch.distributed.all_gather(
                labels_gather, labels, group=mpu.get_data_parallel_group()
            )
            torch.distributed.all_gather(
                predicted_gather, predicted, group=mpu.get_data_parallel_group()
            )
            ...
            if args.task == "CoLA":
                ...
                print_rank_last(
                    " > |epoch: {}| metrics for {}: mcc "
                    "= {} , elapsed time (sec): {:.3f}".format(epoch, name, mcc, elapsed_time)
                )
            else:
                ...
                elapsed_time = time.time() - start_time
                print_rank_last(
                    " > |epoch: {}| metrics for {}: pearson "
                    "= {} spearmanr = {} corr = {} elapsed time (sec): {:.3f}".format(
                        epoch, name, pearson_corr, spearman_corr, corr, elapsed_time
                    )
                )
