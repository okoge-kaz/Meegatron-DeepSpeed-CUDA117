...
class MRPCDataset(GLUEAbstractDataset):
    def __init__(self, name, datapaths, tokenizer, max_seq_length, test_label=0):
        ...
        super().__init__("MRPC", name, datapaths, tokenizer, max_seq_length)
                    ...
                    if row[0].strip() == "index":
                        ...
                        print_rank_0(
                            ...
                            "setting labels to {}".format(
                                row[0].strip(), row[3].strip(), row[4].strip(), self.test_label
                            )
                        )
                    else:
                        assert len(row) == 5
                        print_rank_0(
                            "    reading {}, {}, and {} columns"
                            " ...".format(row[0].strip(), row[3].strip(), row[4].strip())
                        )
                    continue
