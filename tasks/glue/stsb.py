...
class STSBDataset(GLUEAbstractDataset):
    def __init__(self, name, datapaths, tokenizer, max_seq_length, test_label=0.0):
        self.test_label = test_label
        super().__init__("STS-B", name, datapaths, tokenizer, max_seq_length)
                    ...
                    if len(row) == 9:
                        ...
                        print_rank_0(
                            ...
                            "setting labels to {}".format(
                                row[0].strip(), row[7].strip(), row[8].strip(), self.test_label
                            )
                        )
                    else:
                        assert len(row) == 10
                        print_rank_0(
                            "    reading {}, {}, {}, and {} columns"
                            " ...".format(
                                row[0].strip(), row[7].strip(), row[8].strip(), row[-1].strip()
                            )
                        )
                    continue
