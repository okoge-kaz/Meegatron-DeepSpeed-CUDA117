...
class DeepSpeedCheckpoint(object):
    def __init__(self, dir, tp_degree=None, pp_degree=None, no_pp=False):
        ...
        if not self.no_pp:
            self.original_tp_degree = len(
                self._get_files_with_prefix(self.layer_files, f"{LAYER_FILE_PREFIX}01")
            )
        ...
        self.dp_degree = len(self.zero_files) // (
    ...
    def _partition_data(self, data_list, num_partitions):
        ...
        partitions_list = [
            data_list[i : i + partition_size] for i in range(0, num_elems, partition_size)
        ]
