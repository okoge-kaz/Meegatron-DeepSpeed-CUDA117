...
def parse_arguments():
    ...
    parser.add_argument(
        "--input_folder", default=None, type=str, help="Input DeepSpeed Checkpoint folder"
    )
    parser.add_argument(
        "--output_folder", default=None, type=str, help="Output Megatron checkpoint folder"
    )
...
def _create_rank_checkpoint(ds_checkpoint, checkpoint_path, tp_index, pp_index, for_release=False):
    ...
    if pp_index in [0, ds_checkpoint.pp_degree - 1]:
        ...
        if pp_index == ds_checkpoint.pp_degree - 1:
            ...
            new_final_norm_sd = {
                f"{FINAL_LAYER_NORM_KEY}.{key}": value for key, value in final_norm_sd.items()
            }
