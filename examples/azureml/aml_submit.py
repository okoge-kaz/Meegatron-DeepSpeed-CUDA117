...
try:
    ...
    compute_config = AmlCompute.provisioning_configuration(
        vm_size="Standard_ND96amsr_A100_v4", min_nodes=32, max_nodes=32
    )
...
vocab_file_dataset = Dataset.File.from_files(
    "https://s3.amazonaws.com/models.huggingface.co/bert/gpt2-vocab.json"
)
merge_file_dataset = Dataset.File.from_files(
...
megatron_ds_env = Environment.from_docker_build_context(
    name="megatron-ds-curated-acpt",
    docker_build_context=DockerBuildContext.from_local_directory(
        workspace=ws, path=".", dockerfile_path="Dockerfile.dockerfile"
    ),
)
megatron_ds_env.register(ws).build(
    ws
).wait_for_completion()  # Comment this out if environment already exists
...
run = experiment.submit(
    megatron_ds_src, tags={"bs": micro_batch_size, "gpus": total_processes_count}
)
