...
def gather_MNLI_results(result_path):
        ...
        if file.startswith("MNLI"):
            ...
            if os.path.exists(fname):
                ...
                if len(overall_candidate) > 0:
                    assert len(overall_candidate) == len(matched_candidate) and len(
                        overall_candidate
                    ) == len(mismatched_candidate)
    ...
    if len(overall) > 0:
        ...
        print(
            f"MNLI how Megatron paper reported: overall results median {statistics.median(overall)}, corresponding matched/mismatched: {matched[median_idx]}/{mismatched[median_idx]}"
        )
        print(f"MNLI other results:")
        print(
            f"MNLI overall results {overall}, median {statistics.median(overall)} (corresponding matched/mismatched {matched[median_idx]}/{mismatched[median_idx]}), mean {statistics.mean(overall)}, std {statistics.stdev(overall)}"
        )
        print(
            f"MNLI matched results {matched}, median {statistics.median(matched)}, mean {statistics.mean(matched)}, std {statistics.stdev(matched)}"
        )
        print(
            f"MNLI mismatched results {mismatched}, median {statistics.median(mismatched)}, mean {statistics.mean(mismatched)}, std {statistics.stdev(mismatched)}"
        )
    else:
...
def gather_QQP_results(result_path):
    ...
    if len(overall) > 0:
        print(
            f"QQP how Megatron paper reported: overall results median {statistics.median(overall)}"
        )
        print(f"QQP other results:")
        print(
            f"QQP overall results {overall}, median {statistics.median(overall)}, mean {statistics.mean(overall)}, std {statistics.stdev(overall)}"
        )
    else:
...
def gather_RACE_results(result_path, task):
    ...
    if len(dev) > 0:
        ...
        print(
            f"RACE-{task} how Megatron paper reported: test result from the median of dev results {test[median_idx]}"
        )
        print(f"RACE-{task} other results:")
        print(
            f"RACE-{task} dev results {dev}, median {statistics.median(dev)}, mean {statistics.mean(dev)}, std {statistics.stdev(dev)}"
        )
        print(
            f"RACE-{task} test results {test}, median {statistics.median(test)}, mean {statistics.mean(test)}, std {statistics.stdev(test)}"
        )
    else:
