...
def gather_GLUE_results(result_path, key, lr):
            ...
            if os.path.exists(fname):
                ...
                if len(overall_candidate) > 0:
                    if len(overall_candidate) != 3:
                        print(
                            f"{result_path} task {key} lr {lr} only has {len(overall_candidate)} epoch"
                        )
    ...
    if len(result) > 0:
        ...
        if key == "MNLI":
            ...
            return (
                round(mnli_matched_result[best_index], 2),
                round(statistics.stdev(mnli_matched_result), 2),
                round(mnli_mismatched_result[best_index], 2),
                round(statistics.stdev(mnli_mismatched_result), 2),
            )
        else:
