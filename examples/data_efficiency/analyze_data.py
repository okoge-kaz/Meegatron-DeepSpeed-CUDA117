...
def get_tasks_args(parser):
    ...
    group.add_argument(
        "--analyzing-num-workers",
        type=int,
        default=1,
        help="Number of workers. Each worker could be a single CPU node.",
    )
    group.add_argument(
        "--analyzing-worker-id", type=int, default=0, help="Worker id of current node."
    )
    group.add_argument(
        "--analyzing-num-threads", type=int, default=1, help="Number of threads for each worker."
    )
    group.add_argument(
        "--analyzing-num-threads-reduce",
        type=int,
        default=1,
...
def train_valid_test_datasets_provider_gpt():
    ...
    train_ds, valid_ds, test_ds = build_train_valid_test_datasets(
        ...
        train_valid_test_num_samples=[
            ...
            1,
            1,
        ],  # Just dummy numbers since we assume args.train_data_exact_num_epochs will override them
...
def metric_total_vocab_freq(data):
    ...
    if args.analyzing_data_type == "BERT":
        ...
        frequency = torch.bincount(data["text"].view(-1), minlength=args.padded_vocab_size + 1)
    return frequency
...
def metric_vocab_rarity(data):
    args = get_args()
    if args.analyzing_data_type == "BERT":
        rarity = torch.sum(data["padding_mask"] * args.total_vocab_freq[data["text"]], dim=1).to(
            torch.long
        )
    elif args.analyzing_data_type == "GPT":
        rarity = []
    ...
    print(
        f"rarity min {min(rarity)}, max {max(rarity)}, len {len(rarity)}, avg {sum(rarity)/len(rarity)}"
    )
    return rarity
...
def metric_seqlen_vocab_rarity(data):
    ...
    metric += torch.sum(data["padding_mask"] * args.total_vocab_freq[data["text"]], dim=1).to(
        torch.long
    )
    print(
        f"metric min {min(metric)}, max {max(metric)}, len {len(metric)}, avg {sum(metric)/len(metric)}"
    )
    return metric
...
def run_map():
    ...
    if args.analyzing_data_type == "BERT":
        ...
        assert (
            "seqlen" not in args.analyzing_metric
        ), "GPT data has fixed seqlen, thus unnecessary to analyze seqlen metric."
        assert (
            "seqlen_vocab_rarity" not in args.analyzing_metric
        ), "GPT data has fixed seqlen, thus unnecessary to analyze seqlen metric."
    if "vocab_rarity" in args.analyzing_metric or "seqlen_vocab_rarity" in args.analyzing_metric:
        ...
        assert os.path.isfile(f"{total_vocab_freq_fname}.bin") and os.path.isfile(
            f"{total_vocab_freq_fname}.idx"
        ), "To analyze vocab rarity, first need to analyze the total vocab freq."
        ...
        if "seqlen_vocab_rarity" in args.analyzing_metric:
            ...
            print(
                f"Metric seqlen_vocab_rarity: using {args.seqlen_coeff} as coefficient for seqlen."
            )
    ...
    data_analyzer = DataAnalyzer(
        ...
        metric_names=args.analyzing_metric,
...
if __name__ == "__main__":
    ...
    if args.analyzing_task == "map":
        ...
        raise NotImplementedError("Task {} is not implemented.".format(args.analyzing_task))
