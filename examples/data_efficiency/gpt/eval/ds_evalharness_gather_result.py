...
def generate_result_table(
    ...
    for i in range(len(configs)):
        ...
        if eval_path is None:
            ...
            assert (
                configs[i][1] in gpt3paper_zeroshoteval_index
            ), "the second element has to be the model size"
            ...
            for file in os.listdir(eval_path):
                if file.endswith(".json"):
                    ...
                    for task in result["results"]:
                        if task != "wikitext":
                            result_dict[task] = [
                                100.0 * result["results"][task][metric_dict[task]]
                            ]
                        else:
    ...
    if find_best:
        ...
        for i in range(len(avg_range)):
            ...
            selected_config[0] = "({})Best Avg{}".format(len(new_configs), avg_tag[i])
        ...
        for i in range(len(candidate_task)):
            ...
            selected_config[0] = "({})Best {}".format(
                len(new_configs), official_dict[candidate_task[i]][0]
            )
            new_configs.append(selected_config)
...
if __name__ == "__main__":
    ...
    configs = [
        [
            ...
            "cosine",
            None,
        ],  # gpt3 paper orig results, thus result path is None
        [
            "(1)repro",
            "125M",
            "300B",
