    ...
    if args.data_efficiency_curriculum_learning:
        if "seqlen_truncate" in data_sampler_state_dict["current_difficulties"]:
            ...
            if current_seqlen < args.seq_length:
                ...
                data["text"] = torch.cat(
                    (
                        data["text"][:, :reshape_len].contiguous().view(-1, current_seqlen + 1),
...
def loss_func(loss_mask, moe_loss, mos_loss, output_tensor):
    ...
    if args.mos or args.kd:
        ...
        print_rank_0(
            ">>> total loss: {}, lm loss {}, kd loss {}".format(loss, averaged_loss[0], mos_loss)
        )
    else:
...
def calculate_mos_loss(args, stu_output, teacher_model, tokens, position_ids, attention_mask):
    ...
    if teacher_model:
        ...
        mos_loss = (
            kd_temp * kd_temp * nn.KLDivLoss(reduction="batchmean")(student_logits, tea_logits)
        )
    ...
    if args.mos or args.kd:
        ...
        if args.teacher_forward and args.teacher_model is not None:
            mos_loss = calculate_mos_loss(
                args, stu_output, args.teacher_model[0], tokens, position_ids, attention_mask
            )
